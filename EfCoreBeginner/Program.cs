...
var fpt = db.Suppliers.First(x => x.Name == "FPT Trading");
var digi = db.Suppliers.First(x => x.Name == "Digiworld");
var petro = db.Suppliers.First(x => x.Name == "Petrosetco");
var suppliers = db.Suppliers.ToList();
foreach (var supplier in suppliers)
{
    Console.WriteLine($"{supplier.Id} - {supplier.Name} - {supplier.Address} - {supplier.Phone}");
}
...
{
    Console.WriteLine($"{product.Name}-{product.Category.Name}-{product.Supplier.Name}");
}
//theninclude là dùng để lấy dữ liệu của các bảng có liên quan và các bảng liên quan đến bảng liên quan

var categoriesthen = db.Categories
    .Include(x => x.Products)
    .ThenInclude(x => x.Supplier)
    .ToList();
foreach (var category in categoriesthen)
{
    Console.WriteLine($"Danh mục: {category.Name}");
    foreach (var product in category.Products)
    {
        Console.WriteLine($"- {product.Name}-{product.Price:N0} VND-{product.Supplier.Name}");
    }
}
//
